...
export const getUserTicket = async (req, res) => {
    try {
        ...
        const tickets = await Ticket.find({ user: userId })
            .poplate(
                "bus",
                "busId from to busType company departure arrival price"
            ).sort({ bookedAt: -1 });
...
export const bookTicket = async (req, res) => {
    try {
        ...
        const unavailableSeats = seatNumber.filter((seatNum) =>
            bus.seats?.some((row) => row.some((seat) => seat.seat_id === seatNum && seat.booked)
            )
        );
        ...
        bus.seats.forEach((row) => {
            row.forEach((seat) => {
                if (seatNumber.includes(seat.seat_id)) {
                    seat.booked = true
                }
            })
        })
